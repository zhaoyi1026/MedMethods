##############################################################################
# MedMethods method module: pathwaylasso
# Pathway Lasso
#
...
### ---- from pathway_lasso/V1/functions.R ----
#=================================
...
### ---- from pathway_lasso/V1/ADMM_adp_functions.R ----
#################################################################
...
# Pathway lasso
pathlasso<-function(X,M,Y,lambda=1,omega=0,phi=1,Phi1=NULL,Phi2=NULL,rho=1,rho.increase=FALSE,standardize=TRUE,tol=1e-10,max.itr=10000,thred=1e-10,est_thred=FALSE,Sigma1=NULL,Sigma2=NULL,
                    trace=FALSE,Theta0=NULL,D0=NULL,alpha0=NULL,beta0=NULL)
{
  n<-length(X)
  p<-ncol(M)
  ...
  if(standardize)
  {
    ...
    re.std<-mediation_net_ADMM_NC(Z=X.std,M=M.std,R=Y.std,lambda=lambda,omega=omega,phi=phi,Phi1=Phi1,Phi2=Phi2,rho=rho,rho.increase=rho.increase,tol=tol,max.itr=max.itr,thred=thred,est_thred=est_thred,
                                  Sigma1=Sigma1,Sigma2=Sigma2,trace=trace,Theta0=Theta0,D0=D0,alpha0=alpha0,beta0=beta0)
    
    A.est<-re.std$A*(M.sd/X.sd)
    B.est<-re.std$B*(Y.sd/M.sd)
    C.est<-re.std$C*(Y.sd/X.sd)
    AB.est<-c(c(A.est)*B.est)
    
    re<-list(lambda=lambda,omega=omega,phi=phi,Phi1=Phi1,Phi2=Phi2,rho=rho,A=A.est,C=C.est,B=B.est,AB=AB.est,
             logLik=mediation_net_ll(as.matrix(X),M,as.matrix(Y),A.est,B.est,C.est,Sigma=diag(rep(1,p+1)))$ll.MR-log(2*pi)*(-(n*(p+1))/2),
             converge=re.std$converge,constraint=re.std$constraint,time=re.std$time,out.scaled=re.std)
  }else
  {
    re<-mediation_net_ADMM_NC(Z=X,M=M,R=Y,lambda=lambda,omega=omega,phi=phi,Phi1=Phi1,Phi2=Phi2,rho=rho,rho.increase=rho.increase,tol=tol,max.itr=max.itr,thred=thred,est_thred=est_thred,
                              Sigma1=Sigma1,Sigma2=Sigma2,trace=trace,Theta0=Theta0,D0=D0,alpha0=alpha0,beta0=beta0)
  }
...
### ---- from pathway_lasso/V1/VSS_adp_functions.R ----
#######################################################################################
